...
1. 在主工作流中進行對話有 `指令模式` 與 `對話模式` 兩種；第一是使用指定格式直接下達指令如 `ABCD-1-13`，指令由三部分組成並以符號 `-` 串接，第一區為參加會議對象，第二區為會議期間，第三區為會議時間，如下範例代表與會者有 `A、B、C、D` 四人，期間 `1` 小時，時間在下午 `1` 點開始，也就是數字 `13`。
...
1. 主腳本；特別注意，若外部工作流命名變更，要更改 `cachedResultName` 的值，或部署後在工作流 `UI` 界面中設置。
